/**
 * Module: web/src/components/HomeView
 * Purpose: Single-view home — health summary tiles, blocking triage, project grid.
 *
...
/**
 * @param {{ projects: object[], loading: boolean }} props
 * @returns {JSX.Element}
 */
export default function HomeView({ projects, loading }) {
  if (loading) {
    return (
  ...
  return (
    <div className="overview-tab">
      ...
      {groups.map(({ folder, projects: groupProjects }, gi) => {
        const offset = groups.slice(0, gi).reduce((s, g) => s + g.projects.length, 0);
        return (
          <div key={folder}>
            {multiGroup && (
