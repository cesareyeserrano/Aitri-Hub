...
export async function cmdSetup(options = {}) {
  ...
  try {
    ...
    if (mode.toLowerCase().startsWith('s')) {
      while (true) {
        const folder = await ask(rl, 'Folder to scan: ');
        if (!folder) { console.log('  Path cannot be empty.'); continue; }
        ...
        console.log(`\nFound ${found.length} Aitri project(s):`);
        found.forEach((p, i) => console.log(`  ${i + 1}. ${p.name}  →  ${p.location}`));
        console.log('');
        const confirm = await ask(rl, `Add all ${found.length} projects? (Y/n) `);
        if (confirm.toLowerCase() === 'n') { console.log('Scan cancelled.'); break; }
