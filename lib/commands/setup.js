...
import { scanFolder } from '../utils/scan.js';
 ...
 *
 * @param {{ scanDir?: string }} [options]
 *   options.scanDir — when provided, skip interactive mode and scan this directory directly.
...
export async function cmdSetup(options = {}) {
  ...
  try {
    ...
    if (options.scanDir) {
      // Non-interactive scan: --scan <dir> was provided
      const resolved = options.scanDir.startsWith('~')
        ? path.join(process.env.HOME ?? '', options.scanDir.slice(1))
        : path.resolve(options.scanDir);

      const found = scanFolder(resolved);
      if (found.length === 0) {
        console.log(`  No Aitri projects found in ${resolved}`);
        console.log(`  (Looking for subdirectories with a .aitri file)\n`);
        return;
      }
      console.log(`\nFound ${found.length} Aitri project(s) in ${path.basename(resolved)}/:`);
      found.forEach((p, i) => console.log(`  ${i + 1}. ${p.name}  ${p.location}`));
      console.log('');
      const confirm = await ask(rl, `Register all ${found.length} project(s)? (Y/n) `);
