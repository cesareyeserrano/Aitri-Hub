...
export async function cmdIntegrationReview(rest) {
  ...
  // FR-046: full pre-release versions are valid review baselines; junk is
  // rejected BEFORE any filesystem/manifest write (TC-046f: manifest untouched).
  /** @aitri-trace FR-ID: FR-046, US-ID: US-046, AC-ID: AC-0462, TC-ID: TC-046f, TC-046h */
  if (!parseSemver(version)) {
    printErr('invalid version', [
      ['value', version],
      ['hint', 'version must be semver, optionally pre-release (e.g. 2.0.0-rc.159)'],
    ]);
  ...
  try {
    ...
    printErr('review failed', [['error', err.message]]);
    return EXIT.CHANGELOG_NOT_FOUND;
  }
  ...
  if (changelog) {
    ...
    if (!resolved.path.startsWith(overrideRoot + path.sep) && resolved.path !== overrideRoot) {
  ...
  try {
    ...
    printErr('failed to write manifest', [['error', err.message]]);
    return EXIT.USAGE;
  }
