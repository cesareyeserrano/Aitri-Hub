...
function getInstalledAitriVersion() {
  ...
  try {
    ...
    // Full-tag capture (FR-040): -rc.N/-alpha.N are part of the version.
    const match = out.match(/v?(\d+\.\d+\.\d+(?:-[0-9A-Za-z-]+(?:\.[0-9A-Za-z-]+)*)?)/);
    const parsed = match ? parseSemver(match[1]) : null;
    _installedAitriVersion = parsed ? parsed.raw : null;
  } catch {
...
/**
 * Returns true if semver `a` is strictly less than semver `b`, with full
 * pre-release precedence (FR-040 SSoT). Unparseable input → false (no
 * fabricated comparison — matches the guard-and-skip posture of callers).
 * @aitri-trace FR-ID: FR-040, US-ID: US-040, AC-ID: AC-0401, TC-ID: TC-040e
 */
...
// FR-044: a corrupt BUGS.json must be visible — its bugs are NOT counted, so
// '0 bugs' on this project proves nothing until the file is fixed.
/** @aitri-trace FR-ID: FR-044, US-ID: US-044, AC-ID: AC-0441, TC-ID: TC-044h */
const ruleBugsParseError = data => {
  const errs = data.bugsSummary?.parseErrors;
  if (!Array.isArray(errs) || errs.length === 0) return null;
  return {
    type: ALERT_TYPE.BUGS_PARSE_ERROR,
    message: `BUGS.json unreadable in ${errs.join(', ')} — bugs are NOT counted`,
    severity: SEVERITY.WARNING,
    command: null,
  };
};

// FR-045: test results not bound to a real verify run — the evidence on disk
// was never produced (or was edited after) the verification spine ran.
/** @aitri-trace FR-ID: FR-045, US-ID: US-045, AC-ID: AC-0451, TC-ID: TC-045h */
const ruleResultsUnbound = data => {
  const rb = data.resultsBinding;
  if (rb !== 'mismatch' && rb !== 'no-stamp' && rb !== 'missing-file') return null;
  return {
    type: ALERT_TYPE.RESULTS_UNBOUND,
    message: `test results not bound to a verify run (${rb})`,
    severity: SEVERITY.WARNING,
    command: 'aitri verify-run',
  };
};
...
const RULES = [
  ...
  // Bug alerts (FR-018)
  ruleOpenBugs,
  // Contract-catchup rc.159: corrupt bug files + unbound results (FR-044/FR-045)
  ruleBugsParseError,
  ruleResultsUnbound,
  // External signals
  ruleExternalSignals,
