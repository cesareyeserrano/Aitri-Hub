 ...
 * Compares major.minor.patch numerically. Non-semver strings return false.
 *
 * @param {string} a
 * @param {string} b
 * @returns {boolean}
...
/**
 * @typedef {{ type: string, message: string, severity: string, command: string|null, tool?: string, signalType?: string }} Alert
 */
...
export function evaluateAlerts(data) {
  ...
  // If the project is ahead of or equal to the CLI, no warning is needed.
  const projectVersion   = data.aitriState?.aitriVersion ?? null;
  const installedVersion = getInstalledAitriVersion();
  if (
    projectVersion   !== null &&
    installedVersion !== null &&
    isVersionLessThan(projectVersion, installedVersion)
  ) {
    alerts.push({
  ...
  // Events use the `event` field (current .aitri schema); `type` is checked as
  // a legacy fallback for snapshots written by very old Aitri versions.
  ...
  // Rule 8b: PHASE_STALLED — project has been on the same phase for > 14 days.
  // Detect by finding the most recent event at a phase < currentPhase (i.e. the transition
  // into the current phase). If no prior events exist, use createdAt.
  // Does not fire if the project has all 5 phases approved (pipeline is complete).
  {
    const currentPhase = data.aitriState?.currentPhase ?? null;
    const approved = data.aitriState?.approvedPhases ?? [];
    const pipelineComplete = approved.length >= 5;

    if (currentPhase !== null && !pipelineComplete) {
      // Find the latest event that preceded the current phase
      ...
      const enteredAt = priorEvents.length > 0
        ? priorEvents[0]
        : new Date(data.aitriState?.createdAt ?? 0).getTime();

      const daysInPhase = enteredAt > 0
        ? (Date.now() - enteredAt) / (24 * 60 * 60 * 1000)
        : null;

      if (daysInPhase !== null && daysInPhase > 14) {
        alerts.push({
  ...
  // ── Bug alerts (FR-018) ─────────────────────────────────────────
  // Rule 16: Open critical/high bugs → blocking. Rule 17: Open medium/low only → warning.
  // @aitri-trace FR-ID: FR-018, US-ID: US-018, AC-ID: AC-035, AC-036, TC-ID: TC-018h, TC-018f, TC-018e
  if (data.bugsSummary && data.bugsSummary.open > 0) {
    const { critical, high, medium, low } = data.bugsSummary;
  ...
  // ── External signals (rule 18+) ─────────────────────────────────
  // Pass through signals from spec/06_EXTERNAL_SIGNALS.json as-is.
  // Each signal becomes one alert — tool owns severity and message.
  if (data.externalSignals?.available === true) {
    for (const signal of data.externalSignals.signals) {
      alerts.push({
        type:     ALERT_TYPE.EXTERNAL_SIGNAL,
        message:  `[${signal.tool}] ${signal.message}`,
        severity: signal.severity,   // 'blocking'|'warning'|'info' — validated by reader
