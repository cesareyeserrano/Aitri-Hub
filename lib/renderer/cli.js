...
function renderCard(project, terminalWidth) {
  ...
  if (project.alerts?.length > 0) {
    const blocking = project.alerts.filter(a => a.severity === 'blocking');
    const warnings = project.alerts.filter(a => a.severity === 'warning');

    if (blocking.length > 0) {
      const msgs = blocking.map(a => {
        const cmd = a.command ? dim(`  →  ${a.command}`) : '';
        return `${red('✖ BLOCKING')} ${a.message}${cmd}`;
      }).join('\n' + indent + '         ');
      lines.push(`${indent}Issues: ${msgs}`);
    }
    if (warnings.length > 0) {
      const msgs = warnings.map(a => {
        const cmd = a.command ? dim(`  →  ${a.command}`) : '';
        return `${yellow('⚠')} ${a.message}${cmd}`;
      }).join(dim(' · '));
      lines.push(`${indent}Warns:  ${msgs}`);
    }
  }
...
/**
 * Group projects by their parent folder name.
 * Remote projects are grouped under 'remote'.
...
function groupByFolder(projects) {
  ...
  for (const p of projects) {
    const folder = p.group
      ?? (p.type === 'remote' ? 'remote' : path.basename(path.dirname(p.location)));
...
export function renderDashboard(data, terminalWidth = 80) {
  ...
  const scanDir  = process.env.AITRI_HUB_SCAN_DIR
    ? dim(` · auto-scan: ${process.env.AITRI_HUB_SCAN_DIR.split(',').map(d => d.trim()).join(', ')}`)
    : '';
  ...
  // Project cards grouped by folder
  const groups = groupByFolder(projects);
  const multiGroup = groups.length > 1;
  for (const group of groups) {
    if (multiGroup) {
      lines.push('');
      lines.push(`  ${dim('▸ ' + group.folder + '/')}`);
    }
