...
export function readManifest() {
  ...
  try {
    ...
    appendLog(`WARN integration-compat.json unreadable (${err.code ?? 'UNKNOWN'}) — treating as absent`);
    return { status: 'absent', data: null };
  }
  let parsed;
  try {
...
export function writeManifest(manifest) {
  ...
  try {
    ...
    throw new Error(`Failed to write integration-compat.json: ${e.message}`);
  }
}
