...
import { PROJECTS_FILE, SCHEMA_VERSION } from '../constants.js';
...
export function writeProjects(config) {
  ...
  try {
    ...
    throw new Error(`Failed to write projects.json: ${err.message}`);
  }
}
...
export function validateRemoteUrl(url) {
  if (!/^(https?|git|file):\/\/.+/.test(url)) {
    return { valid: false, reason: 'Remote location must start with https://, http://, git://, or file://.' };
  }
  return { valid: true };
}
