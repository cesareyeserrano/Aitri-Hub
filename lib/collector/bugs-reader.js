...
export function readBugsSummary(projectDir, artifactsDir) {
  ...
  let raw;
  try { raw = fs.readFileSync(bugsPath, 'utf8'); } catch { return null; }
  ...
  const summary = { open: 0, fixed: 0, verified: 0, closed: 0,
  ...
  for (const b of bugs) {
    ...
    if (status === 'open')     { summary.open++;     summary.openIds.push(b.id ?? ''); }
