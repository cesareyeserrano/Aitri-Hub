...
export function readBugsSummary(projectDir, artifactsDir) {
  ...
  try {
    ...
    return null; // absent file is normal — no bugs recorded yet
  }

  // FR-044: a BUGS.json that EXISTS but cannot be parsed (bad JSON or a shape
  // that violates the contract) must be distinguishable from zero bugs. The
  // counters degrade to zero (display never crashes) but parseErrors names the
  // scope so the alert engine and UI can flag it.
  /** @aitri-trace FR-ID: FR-044, US-ID: US-044, AC-ID: AC-0441, TC-ID: TC-044f */
  ...
  try {
    ...
    return parseErrorSummary();
  }

  const bugs = parsed?.bugs;
  if (!Array.isArray(bugs)) return parseErrorSummary();
