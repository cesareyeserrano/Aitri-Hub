...
export async function collectOne(project) {
  ...
  if (snapshotEligible) {
    ...
    if (result.ok) {
      ...
      // Snapshot does not yet expose lastSession; pull it inline so the
      // snapshot path can still render FR-016 without invoking readAitriState.
      // rc.51+ projects keep it in per-machine .aitri.local (FR-042) — for a
      // LOCAL project that file is this machine's own state, so reading it for
      // display is the deliberate Hub deviation from SCHEMA.md's blanket
      // "never read .aitri.local" (recorded in BUILD_PLAN; Core feedback filed
      // to expose lastSession on status --json and retire this inline read).
