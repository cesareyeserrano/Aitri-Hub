...
import { SCHEMA_VERSION, CACHE_DIR, STATUS, GIT_TIMEOUT_MS, FALLBACK_BASELINE } from '../constants.js';
...
export async function collectOne(project) {
  ...
  const versionTooOld  = detectedVer === null
    || semverCmp(detectedVer, SNAPSHOT_MIN_AITRI_VERSION) < 0;
  ...
  if (snapshotEligible) {
    ...
    if (result.ok) {
      ...
      if (lastSession === null && aitriRaw?.lastSession && typeof aitriRaw.lastSession === 'object') {
        lastSession = {
...
export async function collectAll(projects) {
  ...
  const effectiveReviewedUpTo = manifest.status === 'valid'
    ? manifest.data.reviewedUpTo
    : FALLBACK_BASELINE;
  ...
  const integrationAlert = evaluateIntegrationAlert(
    detectedAitriVersion,
    effectiveReviewedUpTo,
    {
