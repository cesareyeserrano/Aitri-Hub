  ...
  for (const p of perPipeline) {
    ...
    const gates = Array.isArray(p.quality_gates)
      ? p.quality_gates.map(g => ({
          ...
          // Truthy, not === true: Core's verify-complete gate reads required
          // truthy, and Core itself coerces with !! before emitting — mirror it
          // so a future Core regression passing raw artifact values through
          // could never render a blocking gate as "advisory" here.
          required: !!g?.required,
