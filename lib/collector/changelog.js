...
export function extractSection(changelog, version) {
  ...
  if (typeof version !== 'string' || !version) throw err('InvalidArg', 'version must be a non-empty string');

  const lines = changelog.split('\n');
  const headingRe = new RegExp(`^##\\s+${version.replace(/\./g, '\\.')}(?:\\s|$)`);
  let start = -1;
