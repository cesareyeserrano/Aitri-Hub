...
export async function checkRemoteChanged(projectId, location) {
  ...
  const now = Date.now();
  const entry = _state.get(projectId) ?? { lastCheckedAt: 0, lastUpdatedAt: null, backoffUntil: 0, branch: null };
  ...
  _state.set(projectId, { lastCheckedAt: now, lastUpdatedAt: updatedAt ?? entry.lastUpdatedAt, backoffUntil: 0, branch });
