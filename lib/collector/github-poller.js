...
// Hard cap on the body of any HTTPS response we read. The .aitri file is a
// small JSON config in practice; anything larger than this is either a
// misconfiguration or a hostile public repo. Without a cap, every collection
// cycle (every REFRESH_MS) would buffer the whole payload into memory.
...
/**
 * Drop poller state for any project that is no longer in the active list.
 * Caller (collector) passes the set of currently registered project ids on
 * every cycle; this prevents the in-memory map from growing unbounded as
 * projects are added and removed over a long-running aitri-hub web session.
 *
 * @param {Iterable<string>} activeIds
 * @returns {number} how many entries were evicted
 */
export function prunePollerState(activeIds) {
 ...
 */
/**
 * Test-only: inject poller state for a project so throttle/backoff paths are
 * testable without network (mirror of _resetPollerState).
 * @param {string} projectId
 * @param {{lastCheckedAt?:number,lastUpdatedAt?:string|null,backoffUntil?:number,branch?:string|null}} entry
 */
export function _setPollerState(projectId, entry) {
  _state.set(projectId, {
    lastCheckedAt: 0,
    lastUpdatedAt: null,
    backoffUntil: 0,
